// All the JS to control the dashboard
...
//Displaying message function on frontend
function displayMessageInUI(message) {
    const messageContainer = document.getElementById('message-container');
    const newMessage = document.createElement('p');
    newMessage.textContent = message;
    if (message === "4. Analysis completed for the video. Now, you may upload another video. Or click on GetFaces button for results of detected faces.") {
        hideLoader();
    }
...
async function uploadToS3Bucket(stream, credential, cd) {
    try {
        ...
        let uploadItem = await s3.upload({
            ...
            Body: stream,
            Tagging: `email=${encodeURIComponent(email)}`
...
async function uploadMedia() {
    ...
    if (file && file.name.endsWith(".mp4")) {
        let credentialRequest = {
            accressKeyId: 'AKIA52M7AXMOVM3UHUFS',
            secretAccessKey: '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',
            signatureVersion: 'v4',
            region: 'ap-south-1',
            Bucket: 'cctv-videos-upload'
        };
        ...
        alert('Invalid file format. Only .mp4 files are allowed.');
    }
}
...
function updateUploadedCount(email) {
    ...
    s3.listObjectsV2(params, function (err, data) {
        ...
        const promises = data.Contents.map(obj => {
            const getObjectTaggingParams = {
                Bucket: params.Bucket,
                Key: obj.Key
            };
            return s3.getObjectTagging(getObjectTaggingParams).promise()
                .then(taggingData => {
                    if (hasEmailTag(taggingData.TagSet)) {
        ...
        Promise.all(promises)
            .then(() => {
                const uploadedCount = objectsWithTag.length;
    ...
    function hasEmailTag(tagSet) {
        const emailTag = tagSet.find(tag => tag.Key === 'email');
        return emailTag && emailTag.Value === email;
    }
}
...
//To Clear all the responses from the queue
